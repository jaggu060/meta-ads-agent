    ...
    <div style={S.field}>
      ...
      {!media ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div onClick={() => imgRef.current.click()} style={{ border: '1px dashed var(--border2)', borderRadius: 'var(--radius-sm)', padding: '20px 10px', textAlign: 'center', cursor: 'pointer', background: 'var(--bg3)' }}>
            ...
            <input ref={imgRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => handleFile(e.target.files[0], 'image')} />
          </div>
          <div onClick={() => vidRef.current.click()} style={{ border: '1px dashed var(--border2)', borderRadius: 'var(--radius-sm)', padding: '20px 10px', textAlign: 'center', cursor: 'pointer', background: 'var(--bg3)' }}>
            ...
            <input ref={vidRef} type="file" accept="video/*" style={{ display: 'none' }} onChange={e => handleFile(e.target.files[0], 'video')} />
          </div>
        </div>
      ) : (
        <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '12px' }}>
          {media.type === 'image'
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 600 }}>{media.name}</div>
              <div style={{ fontSize: 11, color: 'var(--text3)' }}>{media.size} MB · {media.type}</div>
            </div>
            <button onClick={() => setMedia(null)} style={{ background: 'var(--bg4)', border: '1px solid var(--border)', color: 'var(--text2)', padding: '6px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600 }}>Remove</button>
          </div>
        </div>
      )}
    </div>
...
{"campaignName":"string","objective":"REACH","headline":"string max 40 chars","primaryText":"2-3 sentences","cta":"string","targeting":{"ageMin":18,"ageMax":45,"interests":["i1","i2","i3"],"locations":["city1","city2"]},"estimatedReach":"string","tips":["t1","t2","t3"]}`
      let result
      if (isDemo()) {
  ...
  const launch = async () => {
    ...
    try {
      if (!isDemo()) {
        await metaCall('createCampaign', { name: draft.campaignName, objective: draft.objective, daily_budget: parseInt(brief.budget) * 100, status: 'PAUSED' })
      } else {
        await new Promise(r => setTimeout(r, 1200))
        const saved = JSON.parse(localStorage.getItem('demo_campaigns') || '[]')
        saved.push({ id: Date.now(), name: draft.campaignName, status: 'PAUSED', objective: draft.objective, daily_budget: parseInt(brief.budget) * 100 })
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 28 }}>
        {steps.map((s, i) => <div key={s} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= step ? 'var(--accent)' : 'var(--border)' }} />)}
      </div>
        ...
        <div>
          <div style={S.card}>
            <div style={S.field}>
              ...
              <textarea rows={3} placeholder="e.g. My bakery in Pune — handmade cakes for birthdays" value={brief.product} onChange={e => set('product', e.target.value)} style={{ resize: 'none' }} />
            </div>
            ...
            <div style={S.field}>
              ...
              <select value={brief.goal} onChange={e => set('goal', e.target.value)}>
                <option value="REACH">Reach — show to as many people as possible</option>
          ...
          <button style={S.primary} onClick={() => { setStep(1); generateDraft() }} disabled={!brief.product || loading}>Generate AI Draft →</button>
        </div>
      )}
      ...
      {step === 2 && draft && (
        <div>
          ...
          <div style={S.card}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 700, letterSpacing: 2, marginBottom: 6, textTransform: 'uppercase' }}>Headline</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div><div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 3 }}>CTA</div><div style={{ fontWeight: 700, color: 'var(--blue)' }}>{draft.cta}</div></div>
              <div style={{ textAlign: 'right' }}><div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 3 }}>Est. Reach/day</div><div style={{ fontWeight: 700, color: 'var(--green)' }}>{draft.estimatedReach}</div></div>
            </div>
          </div>
          <div style={S.card}>
            ...
            <div>{draft.targeting.locations.map(l => <span key={l} style={{ ...S.tag, color: 'var(--blue)' }}>{l}</span>)}</div>
          </div>
          <div style={{ ...S.card, border: '1px solid rgba(0,196,140,0.2)', background: 'rgba(0,196,140,0.04)' }}>
            ...
            {draft.tips.map((t, i) => <div key={i} style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 6, paddingLeft: 12, borderLeft: '2px solid var(--green)', lineHeight: 1.5 }}>{t}</div>)}
          </div>
          <button style={S.primary} onClick={launch} disabled={loading}>{loading ? 'Launching...' : '🚀 Confirm & Launch'}</button>
      ...
      {step === 3 && (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text3)', lineHeight: 1.7, marginBottom: 32 }}>Saved in Meta Ads Manager as PAUSED. Go there to review and publish.</div>
